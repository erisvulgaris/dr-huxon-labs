...
export function ProfileView() {
  ...
  const { route, setRoute, setQuickView, compareIds } = useNav();
  const wishCount = useWishlist((s) => s.ids.length);
  const subCount = useSubscriptions((s) => s.subscriptions.length);

  const activeTab = route === "explore" ? "explore" : route === "rewards" ? "rewards" : "profile";

  return (
    <div className="pb-8">
      {/* Sub Tab Bar */}
      <div className="px-4 pt-4">
        <div className="mb-5 flex gap-1 rounded-2xl bg-[oklch(var(--glass-tint)/0.04)] border border-[oklch(var(--glass-tint)/0.06)] p-1">
          {[
            { id: "profile", label: "Dashboard" },
            { id: "explore", label: "Wellness" },
            ...
            return (
              <button
                key={tab.id}
                onClick={() => setRoute(tab.id as any)}
                className={cn(
                  "relative flex-1 rounded-xl py-2 text-center text-[12px] font-semibold transition-colors pressable",
                  isActive ? "text-text-gold" : "text-muted-foreground hover:text-foreground"
                )}
              >
                {isActive && (
                  <motion.span
                    layoutId="profile-subtab-bg"
                    className="absolute inset-0 rounded-xl bg-[oklch(var(--gold)/0.12)] border border-[oklch(var(--gold)/0.2)]"
